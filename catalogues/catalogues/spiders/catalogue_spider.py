...
def get_next_monday():
    ...
    days_until_monday = 7 - cur_time.weekday() if cur_time.weekday() > 0 else 0
    next_monday = cur_time + timedelta(days=days_until_monday)
...
CATALOGUE_PATH = abspath(
    join(PATH["download_path"], "images", "{0}".format(get_next_monday()))
)
...
def check_last_page_exists(self, response, catalogue_page, first_page):
        page_numbers = response.xpath('//div[@class="numbers"]/a/text()').extract()
        if not page_numbers:
            self.logger.warning("No page numbers found")
            return
        try:
            page_count = int(page_numbers[-1]) - 1
        except (ValueError, IndexError):
            self.logger.warning("Could not parse page count")
            return
        ...
        if self.check_catalogue_exists(catalogue_page["name"], last_page_url) == False:
            ...
            next_page = response.xpath(
                '//div[@class="numbers"]/a[@rel="next"]/@href'
            ).extract_first()
            if next_page is not None:
                root_element = response.xpath(
                    '//tr/td[@class="leaflet-detail-big monitoring-leaflet"]'
                )
                ...
                yield scrapy.Request(
                    ...
                    cb_kwargs=dict(
                        ...
                        img_urls=img_urls,
                        last_page_response=last_page_response,
                    ),
                )
    ...
    def parse_au_detail(self, response, catalogue_page, img_urls, last_page_response):
        ...
        if next_page is not None:
            ...
            if (
                self.check_catalogue_exists(catalogue_page["name"], last_page_url)
                == False
            ):
                header_text = last_page_response.xpath("//div[@class='header']/h1/text()").get()
                if header_text is None:
                    last_page_name = "unknown"
                else:
                    last_page_name = "-".join(
                        header_text.strip()
                        .replace("\n", "")
                ...
                self.write_to_file(
                    catalogue_page, last_page_name, img_urls, last_page_url
                )
    ...
    def write_to_file(self, catalogue_page, last_page_name, img_urls, last_page_url):
        ...
        with open(
            join(CATALOGUE_INFO_PATH, catalogue_page["name"] + ".csv"),
            "a",
            encoding="utf-8",
            newline="",
        ) as im_file:
            ...
            self.download_imgs[last_page_url] = [
                catalogue_page["name"],
                str(datetime.now().date()),
                last_page_name,
                img_urls,
    ...
    def closed(self, reason):
        ...
        if self.error_cata == 0:
            ...
            print("\t----------------------------------------------------")
            print(
                "\t--- DOWNLOAD COMPLETED WITH {0} ERROR CATALOGUE(S) ---".format(
                    self.error_cata
                )
            )
        ...
        for last_page_url, download_img in self.download_imgs.items():
            try:
                ...
                self.write_error_history(
                    download_img[0], last_page_url, str(e).replace("\n", " ")
                )
                self.error_cata += 1
    ...
    def write_error_history(self, cata_name, last_page_url, exception_detail):
        with open(DOWNLOAD_ERROR_PATH, "a", encoding="utf-8", newline="") as err_file:
            ...
            err_writer.writerow(
                [str(datetime.now().date()), cata_name, last_page_url, exception_detail]
            )
    ...
    def download_jpg_to_pdf(self, download_img, cata_name, output_path):
        ...
        for img_url in download_img[3]:
            ...
            # --------------
            r = requests.get(
                img_url, stream=True, headers={"User-agent": "Mozilla/5.0"}
            )
    ...
    def images_to_pdf(self, output_path, cata_name):
        with open(join(output_path, cata_name + ".pdf"), "wb") as pdf_file:
            pdf_file.write(
                img2pdf.convert(
                    [
                        join(output_path, img)
                        for img in listdir(output_path)
                        if img.endswith(".jpg")
                    ]
                )
            )
