...
CATALOGUE_PATH = abspath(
    join(PATH["download_path"], "test_images", "{0}".format(get_next_monday()))
)
...
class CatalogueSpider(scrapy.Spider):
    ...
    def check_last_page_exists(self, response, catalogue_page, first_page):
        page_count = (
            int(response.xpath('//div[@class="numbers"]/a/text()').extract()[-1]) - 1
        )
    ...
    def parse_au_detail(self, response, catalogue_page, img_urls, last_page_response):
        ...
        if next_page is not None:
            ...
            if (
                ...
                header_text = last_page_response.xpath(
                    "//div[@class='header']/h1/text()"
                ).get()
